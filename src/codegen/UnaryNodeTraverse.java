...
public class UnaryNodeTraverse extends BaseTraverse implements GeneratorReceiver {
    ...
    public Object traverse(AstNode root) {
        ...
        switch (production) {
            ...
            case SyntaxProductionInit.Unary_LP_ARGS_RP_TO_Unary:
                 ...
                 */
                if (funcName != "" && funcName.equals(BaseTraverse.funcName)) {
                    reEntry = true;
                }
                ...
                if (func != null) {
                     ...
                     */
                    BaseTraverse.funcName = funcName;
                    int count = 0;
                    ...
                    if (BaseTraverse.isCompileMode && !reEntry) {
                        /*
                         * 在编译状态下，遇到函数自我递归调用则不需要再次为函数生成代码，只需要生成invoke指令即可
                         */
                        Traverse traverse = TraverseFactory.getInstance().getTraverse(func);
                        Generator.getInstance().setInstructionBuffered(true);
                        traverse.traverse(func);
                        symbol = (Symbol) root.getChildren().get(0).getAttribute(NodeKey.SYMBOL);
                        emitReturnInstruction(symbol);
                        Generator.getInstance().emitDirective(Directive.END_METHOD);
                        Generator.getInstance().setInstructionBuffered(false);
                        Generator.getInstance().popFuncName();
                    }
            ...
            case SyntaxProductionInit.Unary_StructOP_Name_TO_Unary:
                ...
                if (isStructArray == true) {
                    ...
                    structArray.addScope(Generator.getInstance().getCurrentFuncName());
                } else {
                    symbol.addScope(Generator.getInstance().getCurrentFuncName());
                }
                //如果是第一次访问结构体成员变量,那么将结构体声明成一个类
                Generator.getInstance().putStructToClassDeclaration(symbol);
                ...
                if (args.getValue() != null) {
                    Generator.getInstance().readValueFromStructMember(symbol, args);
                }
                ...
                if (isSymbolStructPointer(symbol)) {
                    ...
                    GeneratorBrocasterImpl.getInstance().registerReceiverForAfterExe(this);
                } else {
    ...
    private Symbol getStructSymbolFromStructArray(Object object) {
        ...
        Generator.getInstance().createStructArray(symbol);
        ...
        if (struct == null) {
            ...
            try {
                ...
                Generator.getInstance().createInstanceForStructArray(symbol, idx);
            } catch (Exception e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
    ...
    private void emitReturnInstruction(Symbol symbol) {
        if (symbol.hasType(Specifier.INT)) {
            Generator.getInstance().emit(Instruction.IRETURN);
        } else {
            Generator.getInstance().emit(Instruction.RETURN);
        }
    }
    ...
    @Override
    public void handleGeneratorMessage(AstNode code) {
